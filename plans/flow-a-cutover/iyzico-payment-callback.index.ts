...
// HTML YANIT
// ═════════════════════════════════════════════════════════════════
// Sayfaya basilan her metin once kacislanir. message/extraInfo bazi
// yollarda iyzico'dan gelen errorMessage'i tasiyor; kacislama olmadan
// yansitilan XSS'e acik olurdu.
...
function htmlResponse(title: string, message: string, isSuccess: boolean, extraInfo?: string): Response {
  ...
  const safeExtra = extraInfo ? escapeHtml(extraInfo) : undefined;
...
<body>
  <div class="container">
    ...
    <h1>${safeTitle}</h1>
    <p>${safeMessage}</p>
    ${safeExtra ? `<p class="extra">${safeExtra}</p>` : ''}
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (txn.iyzico_environment && txn.iyzico_environment !== IYZICO_ENV) {
      ...
      await supabase.rpc("atomic_finalize_payment_transaction", {
    ...
    if (!isPaymentSuccess && iyzicoResult.status === "success") {
      ...
      const { error: rpcErr } = await supabase.rpc("atomic_finalize_payment_transaction", {
        p_transaction_id: txn.id,
        p_payment_id: iyzicoResult.paymentId,
    ...
    // Ic hata detayi yalnizca loga; kullaniciya sabit metin.
    console.error("Callback beklenmeyen hata:", error.message);
    return htmlResponse(
      "Hata",
      "Odeme islenirken beklenmeyen bir hata olustu. Destek ekibine bildirin.",
      false,
    );
  }
});
