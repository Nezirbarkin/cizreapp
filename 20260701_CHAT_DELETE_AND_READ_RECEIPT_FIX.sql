-- ============================================================================
-- MIGRATION: 20260701_CHAT_DELETE_AND_READ_RECEIPT_FIX.sql
-- ----------------------------------------------------------------------------
-- SOFT DELETE modeli ile:
--   SORUN-1: kendi sohbetini silince karşı tarafın mesajları da siliniyordu.
--            Artık sadece silen kullanıcının perspective'ini gizler.
--   SORUN-2: kendi mesajı "karşıdan gelmiş" + "karşı baktı" yanılgısı.
--            getMessages tek conversation_id'den çeker; mark_sender_messages_read
--            karşı conversation'a dokunmaz.
--   SORUN-3: notify_direct_message yanlış recipient'a push atıyordu. Düzeltildi.
-- ----------------------------------------------------------------------------
-- GERI ALMA:
--   ALTER TABLE conversations DROP COLUMN IF EXISTS deleted_for_user_id;
--   ALTER TABLE messages DROP COLUMN IF EXISTS deleted_for_user_id;
--   DROP FUNCTION delete_conversation_for_user(UUID);
-- ============================================================================

-- 1) SOFT DELETE KOLONLARI
...
-- 2) delete_conversation_for_user RPC (SOFT DELETE)
...
-- 3) update_conversation_on_message TRIGGER (soft delete uyumlu)
...
BEGIN
    ...
    -- 1) Gonderen tarafin conversation satirini guncelle.
    --    Gonderen bu conversation'i daha once sildiyse, yeni mesaj attiginda
    --    tekrar gorunur yap (kendi sildigi sohbete yazdiysa).
    UPDATE conversations
    SET
    ...
    -- 2) Alici tarafin conversation satirini guncelle veya olustur.
    --    Alicinin gizliligini (deleted_for_user_id) KORU.
    IF NEW.sender_id = v_conv_user_id THEN
        -- Gonderen = conversation.user_id -> alici = v_conv_other_user_id
        ...
        DO UPDATE SET
            ...
            updated_at = NOW();
    ELSE
        -- Gonderen = conversation.other_user_id -> alici = v_conv_user_id
        INSERT INTO conversations (user_id, other_user_id, last_message, last_message_time, unread_count, created_at, updated_at)
...
COMMENT ON FUNCTION public.update_conversation_on_message() IS 'Yeni mesajda her iki tarafin conversation kaydini gunceller; soft delete ile uyumlu; alici tarafin gizliligini korur.';

-- 4) mark_sender_messages_read RPC (karsi conversation'a dokunmaz)
...
BEGIN
    ...
    -- SADECE p_conversation_id icindeki, BENIM (p_reader_id) gonderdigim
    -- mesajlari is_read=true yap. Karsi conversation'a DOKUNMA.
    UPDATE messages
...
-- 5) mark_messages_as_read RPC (tek conversation + reader kontrolü)
...
-- 6) notify_direct_message TRIGGER (dogru recipient secimi)
...
DECLARE
    ...
    v_request_id BIGINT;
BEGIN
    -- Dogru aliciyi bul: conversation satirindan NEW.sender_id OLMAYAN taraf.
    SELECT
        CASE
...
-- 7) conversations_update_own policy garanti (RPC UPDATE icin)
DO $$
BEGIN
    IF NOT EXISTS (
...
DO $$
BEGIN
    RAISE NOTICE '✅ 20260701_CHAT_DELETE_AND_READ_RECEIPT_FIX.sql TAMAMLANDI';
END $$;