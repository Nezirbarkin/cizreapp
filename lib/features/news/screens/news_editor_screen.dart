...
class _NewsEditorScreenState extends State<NewsEditorScreen> {
  ...
  Future<void> _uploadImage() async {
    ...
    try {
      ...
      if (uploadUrl != null) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('? G�rsel ba�ar�yla y�klendi')),
          );
        }
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('? Y�kleme hatas�: $e')),
        );
      }
    } finally {
  ...
  Future<void> _saveNews() async {
    ...
    try {
      if (widget.news == null) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('? Haber ba�ar�yla olu�turuldu')),
          );
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('? Haber ba�ar�yla g�ncellendi')),
          );
          Navigator.pop(context, true);
        }
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.news == null ? '?? Yeni Haber' : '?? Haberi D�zenle'),
        centerTitle: true,
        elevation: 0,
            ...
            children: [
              ...
              TextFormField(
                ...
                decoration: InputDecoration(
                  labelText: 'Ba�l�k *',
                  hintText: 'Haber ba�l���n� girin',
                  prefixIcon: const Icon(Icons.title),
                  border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)),
                ),
                validator: (v) => v?.isEmpty ?? true ? 'Ba�l�k gereklidir' : null,
              ),
              ...
              TextFormField(
                ...
                decoration: InputDecoration(
                  labelText: '��erik *',
                  hintText: 'Haber i�eri�ini girin',
                  prefixIcon: const Icon(Icons.article),
                  border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)),
                ),
                validator: (v) => v?.isEmpty ?? true ? '��erik gereklidir' : null,
              ),
              const SizedBox(height: 24),
              _buildSectionTitle('??? G�rsel Y�netimi'),
              const SizedBox(height: 12),
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        ...
                        label: const Text('G�rseli Kald�r'),
                      ...
                      children: [
                        Expanded(
                          child: ElevatedButton.icon(
                            ...
                            label: Text(_isUploadingImage ? 'Y�kleniyor...' : '?? Sunucuya Y�kle'),
                          ),
                        ),
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      const SizedBox(height: 12),
                      Text('G�rsel se�ilmedi', style: TextStyle(color: Colors.grey[600])),
                    ],
                  ),
                ),
              const SizedBox(height: 12),
                ...
                children: [
                  Expanded(
                    child: DropdownButtonFormField<String?>(
                      ...
                      items: [
                        const DropdownMenuItem(value: null, child: Text('Se�iniz')),
              ...
              const SizedBox(height: 24),
              _buildSectionTitle('?? Se�enekler'),
              const SizedBox(height: 12),
              CheckboxListTile(
                value: _isPublished,
                onChanged: (v) => setState(() => _isPublished = v ?? false),
                title: const Text('? Yay�nla'),
                subtitle: const Text('Haberi yay�nla'),
                contentPadding: EdgeInsets.zero,
              ),
              CheckboxListTile(
                value: _isFeatured,
                onChanged: (v) => setState(() => _isFeatured = v ?? false),
                title: const Text('? �ne ��kan'),
                subtitle: const Text('Ana sayfada �ne ��kar'),
                contentPadding: EdgeInsets.zero,
              ),
              CheckboxListTile(
                value: _isBreaking,
                ...
                subtitle: const Text('Son dakika haberi olarak i�aretle'),
                contentPadding: EdgeInsets.zero,
              ),
