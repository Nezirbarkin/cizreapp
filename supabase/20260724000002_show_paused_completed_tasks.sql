-- =============================================================================
-- 20260724000002_show_paused_completed_tasks.sql
-- Görevler ekranı sadece status='active' olan görevleri gösteriyordu; admin
-- bir görevi duraklattığında (paused) veya tamamlandığında (completed)
-- kullanıcı tarafında tamamen kayboluyordu. Artık paused/completed görevler
-- de listede kalır (client "Duraklatıldı" / "Tamamlandı" rozetiyle gösterir),
-- sadece draft/archived görevler gizli kalır.
-- =============================================================================
...
BEGIN
  ...
  WHERE t.status IN ('active', 'paused', 'completed')
    AND (t.status <> 'active' OR t.starts_at <= NOW())
    AND (t.expires_at IS NULL OR t.expires_at > NOW() OR t.status <> 'active')
