-- =============================================================================
-- 101 Okey Plus — Ekonomi kancalarının oyun akışına takılması
--   start_okey_hand            : maçın İLK elinde giriş ücretlerini tahsil eder
--   okey_internal_finalize_hand: el istatistiği + maç bitince potu dağıtır
-- =============================================================================
...
BEGIN
  ...
  IF v_room.current_match_id IS NOT NULL THEN
    SELECT m.* INTO v_prev_match FROM public.okey_matches AS m
  ...
  -- EKONOMİ: maçın İLK eli başlarken giriş ücretleri BİR KEZ tahsil edilir
  IF v_hand_no = 1 THEN
    PERFORM public.okey_internal_collect_entry_fees(p_room_id);
  END IF;
  ...
  v_okey_tile := jsonb_build_object(
    ...
    'number', CASE WHEN (v_indicator->>'number')::int = 13
                   THEN 1 ELSE (v_indicator->>'number')::int + 1 END,
    'isFalseJoker', false
  );
...
-- -----------------------------------------------------------------------------
-- finalize: el istatistiği + maç bitince pot dağıtımı
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  LOOP
    ...
    END IF;
    v_hand_scores := jsonb_set(v_hand_scores, ARRAY[v_h.seat_no::text],
                               to_jsonb(v_base * v_multiplier));
  END LOOP;

  v_new_scores := v_match.scores;
  FOR v_seat IN 0..3 LOOP
    v_new_scores := jsonb_set(
      v_new_scores, ARRAY[v_seat::text],
      to_jsonb(
  ...
  -- EKONOMİ: bu elin istatistikleri
  ...
  IF v_max_cum >= v_room.max_score THEN
    ...
    WHERE id = v_room.id;
    -- EKONOMİ: maç bitti — pot kazanana verilir, maç istatistikleri işlenir
