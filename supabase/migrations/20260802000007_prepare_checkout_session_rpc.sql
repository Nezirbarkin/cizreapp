...
--
-- ISTEMCININ GONDEREBILDIGI (güvenli) ALANLAR:
...
CREATE OR REPLACE FUNCTION private.prepare_checkout_session(
  p_items JSONB,
  p_address_id UUID,
...
-- ════════════════════════════════════════════════════════════════════════
REVOKE ALL ON FUNCTION private.prepare_checkout_session(
  JSONB, UUID, TEXT, TEXT, UUID, TEXT, JSONB, UUID
) FROM PUBLIC, anon;

GRANT EXECUTE ON FUNCTION private.prepare_checkout_session(
  JSONB, UUID, TEXT, TEXT, UUID, TEXT, JSONB, UUID
) TO authenticated;

COMMENT ON FUNCTION private.prepare_checkout_session IS
...
DO $$
BEGIN
    RAISE NOTICE '✅ prepare_checkout_session RPC olusturuldu';
