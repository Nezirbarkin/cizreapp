-- ════════════════════════════════════════════════════════════════════════
-- FIX: complete_online_payment artık coupon_id'yi orders'a yazıyor
-- Tarih: 2026-08-01
--
-- SORUN: complete_online_payment coupon_discount'ı orders'a yazıyordu ama
-- coupon_id'yi yazmıyordu. Online siparişlerde orders.coupon_id hep NULL
-- kalıyordu (kupon indirimi doğru, kullanım sayacı use_coupon ile artıyordu
-- ama denormalize coupon_id raporlama için eksikti).
--
-- ÇÖZÜM: Fonksiyon callback_data.order_data'dan coupon_id'yi okuyup
-- orders.coupon_id kolonuna yazar. İstemci (iyzico-payment-init) zaten
-- order_data'ya coupon_id'yi koyuyor. Aşağıdaki Edge Function değişikliği
-- (use_coupon çağrısı) kullanım sayacını tutuyor; bu migration raporlama
-- bütünlüğü için coupon_id'yi orders'a kalıcı yazıyor.
--
-- Idempotent ve race koruması korundu (önceki migration'dan devralındı).
-- ════════════════════════════════════════════════════════════════════════
...
COMMENT ON FUNCTION public.complete_online_payment(UUID) IS
  'iyzico ödeme başarılı callback sonrası siparişi oluşturur. IDEMPOTENT. Gerçek orders/order_items kolon adları kullanılır. coupon_id artık orders''a yazılıyor (2026-08-01). Race condition korumalı.';