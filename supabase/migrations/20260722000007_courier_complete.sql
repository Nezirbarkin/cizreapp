-- Teslimat kartları tablosu
DROP TABLE IF EXISTS delivery_card_options CASCADE;
CREATE TABLE delivery_card_options (
...
DROP TABLE IF EXISTS courier_service_settings CASCADE;
CREATE TABLE courier_service_settings (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
ALTER TABLE courier_service_settings ENABLE ROW LEVEL SECURITY;
ALTER TABLE courier_requests ENABLE ROW LEVEL SECURITY;

-- delivery_card_options: Herkese okunabilir
CREATE POLICY "delivery_card_public_read" ON delivery_card_options
  FOR SELECT USING (is_active = true);

-- courier_service_settings: Herkese okunabilir
CREATE POLICY "courier_settings_public_read" ON courier_service_settings
  FOR SELECT USING (true);

-- courier_requests: Kimisi doğru yaz
