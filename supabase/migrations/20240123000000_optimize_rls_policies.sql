-- ============================================================================
-- CizreApp - Database Performance Optimization
-- ============================================================================
-- Bu migration dosyası Supabase Database Linter tarafından tespit edilen
-- performans uyarılarını düzeltir:
--
-- 1. Auth RLS InitPlan: auth.uid() -> (select auth.uid())
-- 2. Multiple Permissive Policies: Aynı rol/action için birden fazla politika
-- 3. Duplicate Index: Orders tablosunda duplike indeksler
-- ============================================================================

-- ============================================================================
-- 1. DUPLICATE INDEX CLEANUP (Orders Table)
-- ============================================================================

-- Duplike shop index'lerinden birini sil
DROP INDEX IF EXISTS public.idx_orders_shop;

-- Duplike user index'lerinden birini sil
DROP INDEX IF EXISTS public.idx_orders_user;

-- ============================================================================
-- 2. RLS POLICY OPTIMIZATION - AUTH.UID() FIX
-- ============================================================================
...
-- DROP eski politikalar (auth.uid() kullananlar)
...
-- CREATE optimize edilmiş politikalar
...
CREATE POLICY "cart_select_policy" ON public.cart
    FOR SELECT USING (user_id = (select auth.uid()));

CREATE POLICY "cart_insert_policy" ON public.cart
    FOR INSERT WITH CHECK (user_id = (select auth.uid()));

CREATE POLICY "cart_update_policy" ON public.cart
    FOR UPDATE USING (user_id = (select auth.uid()));

CREATE POLICY "cart_delete_policy" ON public.cart
    FOR DELETE USING (user_id = (select auth.uid()));
...
CREATE POLICY "orders_update_policy" ON public.orders
    FOR UPDATE USING (
        ...
        user_id = (select auth.uid())
        OR
        -- Dükkan sahibi sipariş durumunu güncelleyebilir
        EXISTS (
            SELECT 1 FROM public.order_items
            JOIN public.products ON products.id = order_items.product_id
            WHERE order_items.order_id = orders.id
...
CREATE POLICY "shop_subscribers_select_policy" ON public.shop_subscribers
    FOR SELECT USING (
        -- Dükkan sahibi aboneleri görebilir
        EXISTS (
...
-- Zaten optimize edilmiş, ama multiple policy uyarısı için düzeltelim
...
CREATE POLICY "story_views_select_policy" ON public.story_views
    FOR SELECT USING (
        -- Kullanıcı kendi görüntüleme kayıtlarını görebilir
        viewer_id = (select auth.uid())
        OR
        -- Story sahibi görüntüleyenleri görebilir
        EXISTS (
...
CREATE POLICY "support_tickets_select_policy" ON public.support_tickets
    FOR SELECT USING (
        -- Kullanıcı kendi ticket'larını görebilir
        user_id = (select auth.uid())
        OR
...
CREATE POLICY "support_tickets_update_policy" ON public.support_tickets
    FOR UPDATE USING (
        -- Kullanıcı kendi ticket'ını güncelleyebilir
        user_id = (select auth.uid())
        OR
...
-- MIGRATION COMPLETE
-- ============================================================================
-- Bu migration dosyası şunları gerçekleştirdi:
-- 1. Duplicate index'leri kaldırdı (orders tablosu)
-- 2. Tüm RLS politikalarında auth.uid() -> (select auth.uid()) dönüşümü
-- 3. Multiple permissive policies'leri tek bir politikada birleştirdi
-- 4. Politikaları daha mantıksal ve performanslı hale getirdi
-- ============================================================================
