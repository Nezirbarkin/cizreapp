...
void main() {
  group('SMS doğrulama kodu — production güvenliği', () {
    ...
    test(
      'multi_shop_checkout_screen.dart: displayedCode kDebugMode koşulunda',
      () {
        final source = File(
  ...
  group('Sepet join yapısı — discount_price entegrasyonu', () {
    test('CartService.getCart join\'inde discount_price var', () {
      ...
      final joinBlock = source.substring(
        firstTripleQuote,
        secondTripleQuote + 3,
      );
      expect(
        joinBlock.contains('discount_price'),
        isTrue,
    ...
    test('CartService._mapToCartItem productDiscountPrice parse ediyor', () {
      ...
      final endIdx = source.indexOf(
        '}',
        source.indexOf('variantData: item', startIdx),
      );
      final helperBody = source.substring(startIdx, endIdx + 1);
      expect(
        helperBody.contains('productDiscountPrice'),
        isTrue,
  ...
  group('Product model — effectivePrice sözleşmesi', () {
    test('Product.effectivePrice discount_price < price kuralını uygular', () {
      ...
      expect(
        ...
        isTrue,
        reason:
            'Product.effectivePrice discount_price öncelikli mantığı içermeli',
      );
    });
  });
}
